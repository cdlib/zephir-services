...
def query_record_information_from_db(session, ids):
    ...
    for chunk in chunks:
        ...
        query_str = f"""
        SELECT zr.id as htid, 
        zr.cid, 
        zr.namespace,
        zr.source_record_number, 
        attr.name as attr_name, 
        attr.dscr as attr_description, 
        reasons.name as reason_name, 
        reasons.dscr as reason_description, 
        zr.identifiers_json,
        zf.metadata_json
        ...
        LEFT JOIN zephir_filedata as zf ON zf.id = zr.id
        ...
        if result:
            for row in result:
                ...
                # Get information from record's 008
                record = parse_json_to_array(metadata_json)[0]
                record_008 = record.get("008")
                record_008_6_15 = record_008.data[6:15] if record_008 else None
                record_008_28 = record_008.data[28] if record_008 else None
                record_008_17 = record_008.data[17] if record_008 else None

                ordered_dict = OrderedDict()
                ordered_dict["id"] = id
                ordered_dict["cid"] = cid
                ordered_dict["oclcs"] = ",".join(oclcs)
                ordered_dict["008/6-14"] = record_008_6_15
                ordered_dict["008/28"] = record_008_28
                ordered_dict["008/17"] = record_008_17
